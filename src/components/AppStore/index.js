...
class AppStore extends Component {
  ...
  filterList = () => {
    const {tabId, inputValue} = this.state
    const filteredInputValue = appsList.filter(
      item =>
        item.category === tabId &&
        item.appName
          .toLocaleLowerCase()
  ...
  render() {
    ...
    const filteredAppList = this.filterList()
    return (
        ...
        <div className="bg-container">
          ...
          <ul className="tabItem">
            {tabsList.map(item => (
              <TabItem
                key={item.tabId}
                tabsListDetails={item}
                onClickTabList={this.onClickTabList}
          ...
          <ul className="appItem">
            {filteredAppList.map(item => (
              <AppItem key={item.appId} appsListDetails={item} />
            ))}
          </ul>
        </div>
      </div>
